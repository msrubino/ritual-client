...
public partial class RageCamera : MonoBehaviour {
    ...
    public void Initialize( ) {
        ...
        LastOrthoSize = DefaultOrthoSize = _camera.orthographicSize;
    ...
    public void CheckCameraChanges() {
        ...
        if (_camera.orthographic) {
            var currentOrthoSize = _camera.orthographicSize;
        ...
        percentageOfChange = Quaternion.Angle(_camera.transform.rotation, LastCameraRotation) / 100f;
        if (percentageOfChange > UpdateThreshold) {
            LastCameraRotation = _camera.transform.rotation;
